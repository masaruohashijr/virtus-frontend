...
export class MembersListComponent implements OnInit {
  ...
  loadContent(filter: string | null) {
    this._officeService.getMembersByOfficeId(filter, this.office.id, 0, 500).subscribe(resp => {
      this.office.members = resp.content;
  ...
  deleteObject(object: MemberDTO) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.office.members = this.office.members?.filter(item => item.id !== object.id);
        this.objectDataSource.data = this.office.members;
      }
    });
  }
