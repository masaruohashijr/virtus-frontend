...
export class ComponentsGradeListComponent implements OnInit {
  ...
  objectDataSource: MatTableDataSource<ComponentGradeType> = new MatTableDataSource();
  ...
  loadContent(filter: string | null) {
    if (this.component?.componentGradeTypes) {
      if (filter) {
        this.objectDataSource.data = this.component?.componentGradeTypes.filter(it => it.gradeType?.name && it.gradeType?.name.toLocaleLowerCase().includes(filter.toLocaleLowerCase()));
      } else {
  ...
  editObject(object: ComponentGradeType) {
    const dialogRef = this.dialog.open(ComponentsGradeEditComponent, {
      width: '700px',
      data: {
        father: this.component,
  ...
  deleteObject(object: ComponentGradeType) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.component.componentGradeTypes = this.component?.componentGradeTypes?.filter(item => item.id !== object.id);
        this.objectDataSource.data = this.component?.componentGradeTypes;
      }
    });
  }
